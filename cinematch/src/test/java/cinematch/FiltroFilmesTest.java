...
import cinematch.service.FiltroFilmes;
...
class FiltroFilmesTest {
	...
	void setUp() {
		...
		filmeFC = new Filme("F07", "A Chegada", 2016, 116, List.of(Genero.FICCAO_CIENTIFICA, Genero.DRAMA),
				ClassificacaoEtaria.DOZE, Idioma.INGLES, 84);

		filmeTerror = new Filme("F03", "O Iluminado", 1980, 146, List.of(Genero.TERROR), ClassificacaoEtaria.DEZOITO,
				Idioma.INGLES, 88);

		filmeJaAssistido = new Filme("F04", "Interestelar", 2014, 169, List.of(Genero.FICCAO_CIENTIFICA, Genero.DRAMA),
				ClassificacaoEtaria.DOZE, Idioma.INGLES, 95);
		perfil.marcarComoAssistido("F04"); // marca F04 no histórico

		filmeClassificacaoAlta = new Filme("F05", "Tropa de Elite", 2007, 115, List.of(Genero.ACAO, Genero.DRAMA),
				ClassificacaoEtaria.DEZOITO, // > limite 16
				Idioma.PORTUGUES, 80);

		filmeIdiomaErrado = new Filme("F08", "Parasite", 2019, 132, List.of(Genero.DRAMA, Genero.SUSPENSE),
				ClassificacaoEtaria.QUATORZE, Idioma.COREANO, // não aceito
				90);
	}
	...
	void deve_RemoverFilme_Quando_JaFoiAssistido() {
		...
		assertFalse(resultado.contains(filmeJaAssistido), "Filme já assistido não deve aparecer na lista filtrada");
	}
	...
	@ParameterizedTest
	@CsvSource({ "DEZOITO,    DEZESSEIS, false", "DEZESSEIS,  DEZESSEIS,  true", "QUATORZE,  DEZESSEIS,  true" })
	@DisplayName("deve_AceitarOuRejeitar_FilmePorClassificacao")
	void deve_AceitarOuRejeitar_FilmePorClassificacao(ClassificacaoEtaria classificacaoFilme,
			ClassificacaoEtaria classificacaoMaxPerfil, boolean esperaAceitar) {
		...
		Filme filme = new Filme("FT", "Teste", 2024, 120, List.of(Genero.DRAMA), classificacaoFilme, Idioma.INGLES, 50);
	...
	void deve_RemoverFilme_Quando_IdiomaForaDaListaAceita() {
		...
		assertFalse(resultado.contains(filmeIdiomaErrado), "Filme em idioma não aceito deve ser removido");
		assertTrue(resultado.contains(filmeFC));
	}
	...
	void deve_RemoverFilme_Quando_ContemGeneroBloqueadoPeloPerfil() {
		...
		assertFalse(resultado.contains(filmeTerror), "Filme com gênero bloqueado (peso 0.0) deve ser removido");
	}
	...
	void deve_RetornarListaVazia_Quando_TodosFilmesForamFiltrados() {
		List<Filme> catalogoTodosBloqueados = List.of(filmeTerror, // gênero bloqueado
	...
	void deve_ManterApenasFimesElegiveis_Quando_CatalogoMisto() {
		Filme filmeExtra = new Filme("F02", "Ela (Her)", 2013, 126,
				List.of(Genero.FICCAO_CIENTIFICA, Genero.DRAMA, Genero.ROMANCE), ClassificacaoEtaria.DEZESSEIS,
				Idioma.INGLES, 78);

		List<Filme> catalogo = List.of(filmeFC, filmeExtra, // elegíveis
		...
		assertAll(() -> assertEquals(2, resultado.size()), () -> assertTrue(resultado.contains(filmeFC)),
