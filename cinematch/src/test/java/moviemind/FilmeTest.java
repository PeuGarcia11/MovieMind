...
class FilmeTest {
    ...
    void deve_RetornarTodosAtributos_Quando_FilmeCriado() {
        assertAll(
            () -> assertEquals("F01",                    filmeBase.getId()),
            () -> assertEquals("Duna: Parte Dois",       filmeBase.getTitulo()),
            () -> assertEquals(2024,                     filmeBase.getAno()),
            () -> assertEquals(166,                      filmeBase.getDuracaoMinutos()),
    ...
    void deve_ConterGenerosFiccaoCientificaEDrama_Quando_FilmeCriado() {
        ...
        assertAll(
            () -> assertTrue(generos.contains(Genero.FICCAO_CIENTIFICA)),
    ...
    void deve_LancarExcecao_Quando_TentarModificarListaDeGeneros() {
        assertThrows(UnsupportedOperationException.class,
            () -> filmeBase.getGeneros().add(Genero.TERROR));
    }
    ...
    void deve_LancarNullPointer_Quando_IdForNulo() {
        assertThrows(NullPointerException.class, () ->
            new Filme(null, "Titulo", 2024, 100,
                List.of(Genero.DRAMA), ClassificacaoEtaria.LIVRE,
                Idioma.PORTUGUES, 50)
        );
    }
}
