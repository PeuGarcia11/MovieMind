...
import moviemind.service.CalculadoraScore;
...
class CalculadoraScoreTest {
	...
	void deve_RetornarScoreGeneroBaixo_Quando_GeneroNaoPreferido() {
		// perfil não tem Terror configurado: usa neutro 0.5, mas verificamos valor <=
		// 50
	...
	@Test
	@DisplayName("deve_UsarSpy_ParaVerificarChamadasDoCalculo")
	void deve_UsarSpy_ParaVerificarChamadasDoCalculo() {

		Filme filme = filmeComGeneros(List.of(Genero.FICCAO_CIENTIFICA, Genero.DRAMA));

		calculadora.calcularRecomendacao(filme, perfil);

		verify(calculadora, times(1)).calcularScoreGenero(any(Filme.class), any(PerfilCinefilo.class));

		verify(calculadora, times(1)).calcularScoreDuracao(any(Filme.class), any(PerfilCinefilo.class));

		verify(calculadora, times(1)).calcularScoreAfinidade(any(Filme.class), any(PerfilCinefilo.class));
	}
}
