...
/**
 * Aplica regras de exclusão ao catálogo, removendo filmes incompatíveis com o
 * perfil.
 *
 * <p>
 ...
 * <li>Filmes cuja classificação etária ultrapassa o limite do perfil são
 * excluídos.</li>
...
public class FiltroFilmes {
	...
	public List<Filme> filtrar(List<Filme> filmes, PerfilCinefilo perfil) {
		...
		return filmes.stream().filter(filme -> !foiAssistido(filme, perfil))
				.filter(filme -> classificacaoCompativel(filme, perfil)).filter(filme -> idiomaAceito(filme, perfil))
				.filter(filme -> !possuiGeneroBloqueado(filme, perfil)).collect(Collectors.toList());
	}
	...
	/**
	 * Regra 4: se o perfil definiu peso 0.0 para algum gênero presente no filme, o
	 * filme é bloqueado (usuário explicitamente não quer esse gênero).
	 */
	boolean possuiGeneroBloqueado(Filme filme, PerfilCinefilo perfil) {
		return filme.getGeneros().stream().anyMatch(
				genero -> perfil.getPesosGenero().containsKey(genero) && perfil.getPesosGenero().get(genero) == 0.0);
	}
}
