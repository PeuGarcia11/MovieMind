...
public class CalculadoraScore {
    ...
    private static final double NOTA_NEUTRA = 3.0;
    ...
    public Recomendacao calcularRecomendacao(Filme filme, PerfilCinefilo perfil) {
        ...
        double scoreNormalizado =
                Math.max(SCORE_MINIMO,
                        Math.min(SCORE_MAXIMO, scoreFinal));
    ...
    public double calcularScoreDuracao(
        ...
        double fatorPenalidade =
                (double) distanciaMinutos / Math.max(max, 1);

        return Math.max(
                SCORE_MINIMO,
                100.0 - (fatorPenalidade * 100.0)
        );
    }

    public double calcularScoreAfinidade(
            Filme filme,
            PerfilCinefilo perfil
    ) {
    ...
    public List<Recomendacao> gerarRecomendacoes(
        ...
        return filmes.stream()
                .map(filme ->
                        calcularRecomendacao(filme, perfil))
                .sorted((r1, r2) ->
                        Double.compare(
                                r2.getScore(),
                                r1.getScore()
